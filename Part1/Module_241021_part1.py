...
# par0 Huang
p0_huang = {"a1": 1.0, # max rate auto-activation 
           "a2": 1.0,
           "b1": 1.0, # max rate mutual inhibition
           "b2": 1.0,
           "kmi1": 0.5, # dissociation constant, inhibition of x1 by x2
           "kmi2": 0.5, # inhibtion of x2 by x1
           "kaa1": 0.5, # dissociation constant, auto-activation
           "kaa2": 0.5,
           "ktau1": 1.0, # decay rate
           "ktau2": 1.0,
           "n": 4.0
          }

# models
def equations_huang_or_logic(t, var,p):
    '''
    Huang et al. (2007) eq. 1
    '''
    x1 = var[0]
    x2 = var[1]
    
    x1_autoactivation = p["a1"]*x1**p["n"]/(p["kaa1"]**p["n"] + x1**p["n"])
    x1_inhibittion_by_x2 = p["b1"]*p["kmi1"]**p["n"]/(p["kmi1"]**p["n"]+x2**p["n"])
    
    dx1dt = x1_autoactivation + x1_inhibittion_by_x2 - p["ktau1"]*x1
    
    x2_autoactivation = p["a2"]*x2**p["n"]/(p["kaa2"]**p["n"] + x2**p["n"])
    x2_inhibittion_by_x1 = p["b2"]*p["kmi2"]**p["n"]/(p["kmi2"]**p["n"]+x1**p["n"])
    
    dx2dt = x2_autoactivation + x2_inhibittion_by_x1 - p["ktau2"]*x2
    
    return dx1dt, dx2dt

def equations_huang_and_logic(t, var,p):
    '''
    Huang et al. (2007) eq. 1
    '''
    x1 = var[0]
...
def func_bifurcation_diagram_one_par(model_2D, par, key,filename,
    ...
    Last edited 14/11/24: Added key2, allowing two bifurcation parameters to change. key=key2 always. So, effectively one bifurcatoin parameter?
                          I added the option when investigating the kaa-to-kmi ratio in the Huang model.
